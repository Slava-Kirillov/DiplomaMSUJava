...
public class EffectiveScatteringAreaService {
    ...
    public void effectiveScatteringAreaBuild(ComplexVector[] currents, float[][] collocationPoint, float[] cellArea) {

        float[] ESA = new float[181];

        for (int i = 0; i < 181; i++) {
            float angle = i;
            float[] tau = {(float) -Math.cos(angle), (float) -Math.sin(angle), 0.0f};
            ESA[i] = (float) (calcESA(currents, collocationPoint, cellArea, tau)/Math.PI);
        }
        IOUtil.writeResultToFile("ESA.dat", ESA, 1, ESA.length);
    }
